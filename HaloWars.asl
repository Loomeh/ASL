...
split
{
	...
	if(((current.results1 <= 20 && old.results1 >= 20) || (current.results2 <= 20 && old.results2 >= 20)) && !settings["Coop"]){
		return true;
	}
	
	// Split settings for Coop
	if(((current.coopResults1 <= 20 && old.coopResults1 >= 20) || (current.coopResults2 <= 20 && old.coopResults2 >= 20)) && settings["Coop"]){
		return true;
	}
}
...
isLoading
{
	...
	if((current.igt == old.igt || current.results1 <= 20 || current.results2 <= 20 || current.cutscenes != 0 || current.tutorials == 0 || current.playing != 1) && !settings["Coop"]){
		return true;
	}
	
	// Pause the timer when the IGT isn't going on Coop
	if((current.igt == old.igt || current.coopResults1 <= 20 || current.coopResults2 <= 20 || current.cutscenes != 0 || current.tutorials == 0 || current.playing != 1) && settings["Coop"]){
		return true;
	}
}
...
gameTime
{
	...
	if(((current.igt > old.igt && current.results1 >= 20 && current.tutorials != 0 && current.playing == 1) ||
	    (current.igt > old.igt && current.results2 >= 20 && current.tutorials != 0 && current.playing == 1)) && !settings["Coop"]){
		return TimeSpan.FromSeconds(System.Math.Floor(current.igt));
	}
	
	// Show the current IGT on the LiveSplit timer for Coop
	if(((current.igt > old.igt && current.coopResults1 >= 20 && current.tutorials != 0 && current.playing == 1) ||
	    (current.igt > old.igt && current.coopResults2 >= 20 && current.tutorials != 0 && current.playing == 1)) && settings["Coop"]){
		return TimeSpan.FromSeconds(System.Math.Floor(current.igt));
	}
	...
	if(((current.coopResults1 <= 20 && old.coopResults1 >= 20) || (current.coopResults1 <= 20 && old.coopResults1 >= 20)) && settings["Coop"]){
		vars.totalGameTime = System.Math.Floor(vars.totalGameTime + old.igt);
		return TimeSpan.FromSeconds(vars.totalGameTime);
	}
}
