...
split
{
	...
	if((current.results <= 20 && old.results >= 20) && !settings["Coop"]){
		return true;
	}
	
	// Split settings for Coop
	if((current.coopResults <= 20 && old.coopResults >= 20) && settings["Coop"]){
		return true;
	}
}
...
isLoading
{
	...
	if((current.igt == old.igt || current.results <= 20 || current.cutscenes != 0 || current.tutorials == 0 || current.playing != 1) && !settings["Coop"]){
		return true;
	}
	
	// Pause the timer when the IGT isn't going on Coop
	if((current.igt == old.igt || current.coopResults <= 20 || current.cutscenes != 0 || current.tutorials == 0 || current.playing != 1) && settings["Coop"]){
		return true;
	}
}
...
gameTime
{
	...
	if((current.igt > old.igt && current.results >= 20 && current.tutorials != 0 && current.playing == 1) && !settings["Coop"]){
		return TimeSpan.FromSeconds(System.Math.Floor(current.igt));
	}
	
	// Show the current IGT on the LiveSplit timer for Coop
	if((current.igt > old.igt && current.coopResults >= 20 && current.tutorials != 0 && current.playing == 1) && settings["Coop"]){
		return TimeSpan.FromSeconds(System.Math.Floor(current.igt));
	}
